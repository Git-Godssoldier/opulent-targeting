...
These are checkable by reading the copy rather than by calling a tool, so they belong in the same report. Phase 6 of `SKILL.md` owns the copy rules themselves, and `lint_against_style` checks the word-level ones.
