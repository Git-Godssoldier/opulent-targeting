...
Why the rules in Phase 6 of `SKILL.md` are what they are, plus the failure modes worth checking for by name. The numbers live in `email-format-specs.md`.
...
Alt text is functional. Email on Acid states it directly: alt text is not marketing copy, and you should not put a call to action in it or try to sell with it. It replaces the image for a reader who cannot see it, so it describes what the image communicates and stops. If an image needs several sentences of alt text, its content belongs in the body copy instead, and `email-format-specs.md` has the length and the decorative-image rule.

## Spam words are a copy problem, not a deliverability problem

`banned-words.json` is a copy-quality list. It is not a spam filter workaround, and treating it as one gets the priorities backwards.
...
So the rule is: fix the words because the copy is worse with them in, not because they will land the mail in a junk folder. When someone asks whether a subject line will hurt deliverability, the answer lives in Phase 8 (Deliverability, the law, and the send gate), which owns authentication, the sending domain, and the complaint and bounce numbers. Don't answer it from a word list.
...
- The unsubscribe link is hidden or hard to use, which converts people who would have quietly left into people who report the mail as spam. See Phase 8 (Deliverability, the law, and the send gate) for why that number matters.
